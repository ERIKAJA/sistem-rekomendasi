...
novell_df = pd.read_csv("products.csv")
#st.dataframe(novell_df)


#st.dataframe(novell_df.isnull().sum())
novell_df = novell_df[novell_df['product_name'].notnull()]
...
novell_df['desc_clean'] = novell_df['product_name'].apply(clean_text)


#st.dataframe(novell_df)

#print(novell_df.columns)
novell_df.set_index('product_name', inplace=True)
...
novel = st.text_input("Masukkan Nama Produk")
